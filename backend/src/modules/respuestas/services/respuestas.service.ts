...
export class RespuestasService {
  ...
  async registrarRespuestas(
    ...
    if (!registarRespuestasDto.respuestas || registarRespuestasDto.respuestas.length === 0) {
    ...
    if (encuesta.fechaVencimiento && encuesta.fechaVencimiento < new Date()) {
      throw new BadRequestException('La encuesta ha vencido y ya no acepta respuestas');
    }
